...
class BtWifiManager(private val context: Context) {
    ...
    fun enableWifiIfNeeded(onComplete: (Boolean) -> Unit, timeoutMs: Long = 20000L) {
        ...
        Thread {
            // ── 第0步：检查并关闭热点（车机WiFi/热点互斥）──
            if (isHotspotOn()) {
                FileLogger.i(TAG, "检测到热点已开启，车机WiFi/热点互斥，先关闭热点")
                disableHotspotViaReflection()
                Thread.sleep(1000)
                ShellExecutor.exec("settings put global wifi_ap_state 0")
                Thread.sleep(2000)
                ShellExecutor.exec("service call wifi 27 i32 0")
                Thread.sleep(3000)
                if (myTaskId != wifiEnableTaskId) return@Thread
                if (isHotspotOn()) {
                    FileLogger.w(TAG, "热点关闭失败，继续尝试开启WiFi")
                } else {
                    FileLogger.i(TAG, "热点已关闭，开始开启WiFi")
                }
            }

            // ── 方案1: WifiManager API ──
            FileLogger.i(TAG, "方案1: setWifiEnabled(true)")
            ...
            // ── 方案3a: service call wifi 24 s16 <pkg> i32 1 (Android 9格式) ──
            ...
            // ── 方案3b: service call wifi 24 i32 1 (旧格式) ──
            ...
            // ── 方案4: service call wifi 29 i32 1 (备选code) ──
            FileLogger.i(TAG, "方案4: service call wifi 29 i32 1")
            ...
            // ── 方案5: WiFi开关切换（先关再开）──
            FileLogger.i(TAG, "方案5: WiFi开关切换(先关再开)")
            @Suppress("DEPRECATION")
            ...
            Thread.sleep(3000)
            if (myTaskId != wifiEnableTaskId) return@Thread
            if (isWifiEnabled()) { FileLogger.i(TAG, "方案5成功: 开关切换"); return@Thread }
    ...
    private fun isHotspotOn(): Boolean {
        ...
        try {
            ...
            return m.invoke(wifiManager) as Boolean
        } catch (e: Exception) { /* 继续尝试 */ }
        // 反射 getWifiApState == 13 (ENABLED)
        try {
            val m: Method = wifiManager.javaClass.getMethod("getWifiApState")
            val state = m.invoke(wifiManager) as Int
            FileLogger.d(TAG, "getWifiApState=$state")
            return state == 13
        } catch (e: Exception) { /* 继续 */ }
        // Shell 检查
        val result = ShellExecutor.exec("settings get global wifi_ap_state")
        return result.success && result.output.trim() == "13"
    }

    private fun disableHotspotViaReflection() {
        // 反射 setWifiApEnabled(null, false)
        try {
            val m: Method = wifiManager.javaClass.getMethod(
                "setWifiApEnabled",
                android.net.wifi.WifiConfiguration::class.java,
                Boolean::class.javaPrimitiveType
            )
            val ok = m.invoke(wifiManager, null, false) as Boolean
            FileLogger.i(TAG, "反射 setWifiApEnabled(null,false)=$ok")
        } catch (e: Exception) {
            FileLogger.w(TAG, "反射关闭热点失败: ${e.message}")
        }
    }
